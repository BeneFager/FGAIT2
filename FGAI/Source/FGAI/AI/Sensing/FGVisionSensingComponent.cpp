                                              ...
                                              FActorComponentTickFunction* ThisTickFunction)
{
	...
	{
		if (!SensedTargets.Contains(Target) && IsPointVisible(Target->GetTargetOrigin(), Origin, Direction,
		                                                      SensingSettings->DistanceMinimum))
		{
                                               ...
                                               const FVector& Direction, float DistanceMinimum) const
{
	...
	if (bIsValid)
	{
		return IsPlayerVisible(PointToTest, Origin);
	}

	return false;
}

bool UFGVisionSensingComponent::IsPlayerVisible(const FVector& PointToTest, const FVector& Origin) const
{
	FHitResult Hit;
	...
	if (GetWorld()->LineTraceSingleByChannel(Hit, Origin, (PointToTest - Origin) * 2000.f, ECC_GameTraceChannel1, FColParam))
	{
		...
		if (Hit.GetActor()->GetActorLocation() == PointToTest)
		{
			UKismetSystemLibrary::DrawDebugLine(GetWorld(), Origin, (PointToTest - Origin) * 2000.f, FLinearColor::Blue, 10.f);
			UE_LOG(LogTemp, Log, TEXT("eylmao2"));
			return true;
		}
	}
